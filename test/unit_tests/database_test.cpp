...
TEST_F(DatabaseTest, PrepareVariadicFunctionTest) {
    EXPECT_EQ(db2.exec("INSERT INTO excercises (name) VALUES ('Bench Press'), "
                       "('Incline Bench Press')"),
              1);
    EXPECT_EQ(db2.exec("INSERT INTO workouts (workoutName, exOrderNum, excercise, type1, "
...
TEST_F(DatabaseTest, SameDBTest) {
    EXPECT_EQ(db3.exec("INSERT INTO excercises (name, description, muscleGroup, "
...
TEST_F(DatabaseTest, SharedPtrDBTest) {
    EXPECT_EQ(db5->exec("INSERT INTO excercises (name, description, muscleGroup, "
                        "musclesTargeted, type) VALUES ('Lying Leg Raise', 'Lying on "
                        "the ground raise your legs to a 90 degree angle while "
                        "keeping them straight', 'Core', 'Abs', 'reps') "),
              1);
...
TEST_F(DatabaseTest, ThrowTest) {
    ...
    EXPECT_EQ(db1.prepare("INSERT INTO excercises (name) VALUES (?)", "Plank", "Crunch"), -1);
    EXPECT_EQ(db1.prepare(1, "INSERT INTO excercises (name) VALUES (?)", "Plank", "Crunch"), -1);

    // Test preparing query with too few variables to bind
    EXPECT_EQ(db1.prepare("INSERT INTO excercises (name, description) VALUES (?, ?)", "Plank"), 1);
    EXPECT_EQ(db1.prepare(1, "INSERT INTO excercises (name, description) VALUES (?, ?)", "Plank"), 1);
}
...
TEST_F(DatabaseTest, ErrorReturnTest) {
    ...
    EXPECT_EQ(db1.prepare("SELECT * FROM history WHERE excercise = ?", "Plank"), 1);
}
